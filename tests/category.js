module.exports = categoryTesst = () => {
  ...
  describe("Category Test", () => {
    describe("/GET category/", () => {
      ...
      // it("Get All Category - Data not found", async () => { ///gak bakal jumpa
      ...
      it("Gets one category", async () => {
        const res = await request(app).get(`/category/getOne/1`);
        expect(res.statusCode).toEqual(200);
        expect(res.body).toBeInstanceOf(Object);
        expect(res.body.message).toEqual("Success");
      });
      ...
      it("Gets one category - data not found", async () => {
        const res = await request(app).get(`/category/getOne/100`);
        expect(res.statusCode).toEqual(404);
        expect(res.body).toBeInstanceOf(Object);
        expect(res.body.message).toEqual("Data not found");
      });
      ...
      it("update category - internal server error", async () => {
        ...
        const res = await request(app)
          .put(`/category/updateIcon/1`)
          .set("Authorization", `bearer ${token}`)
          .attach("category_icon", "tests/tech.pdf");
      ...
      it("update category - internal server error", async () => {
        ...
        const res = await request(app)
          .put(`/category/updateIcon/1`)
          .set("Authorization", `bearer ${token}`)
          .attach("category_icon", "tests/mega.jpg");
        expect(res.statusCode).toEqual(400);
        expect(res.body).toBeInstanceOf(Object);
      ...
      // it("update category - internal server error", async () => {
      //   const res = await request(app)
      //     .put(`/category/updateIcon/${idTransaction.id}`)
      //     .set("Authorization", `bearer ${token}`)
      //     .send({
